...
let typeFilter = 'hail';   // default: hail pins only — wind circles on demand
let yearFilter = 'all';
...
function renderMap() {
  ...
  const yearOk = ev => yearFilter === 'all'
    || new Date(ev.date + 'T00:00:00').getFullYear() === Number(yearFilter);

  // Wind circles — drawn softly; only when Wind or Everything is selected
  if (typeFilter === 'wind' || typeFilter === 'all') {
    WIND.filter(ev => inWindow(ev.date) && yearOk(ev)).forEach(ev => {
      const radiusMeters = (ev.radius_miles || 6) * 1609.34;
      const circle = L.circle([ev.lat, ev.lng], {
        radius: radiusMeters,
        color: 'rgba(200,164,90,0.4)',
        fillColor: 'rgba(200,164,90,0.05)',
        fillOpacity: 1,
        weight: 1,
        dashArray: '5,4'
      });
  ...
  // Hail pins
  if (typeFilter !== 'wind') {
    HAIL.filter(ev => inWindow(ev.date) && yearOk(ev)).forEach(ev => {
      if (typeFilter === 'golf' && ev.size_in < 1.75) return;
...
function setNoaaBtn(id, ev) {
  const btn = document.getElementById(id);
  if (!btn) return;
  if (ev && ev.report_url) {
    btn.href = ev.report_url;
...
function noaaFindHint(ev, table) {
  // The SPC page is the national log for that day; time (UTC) + location is the lookup key
  const t = ev.time_utc ? ev.time_utc + ' UTC' : '';
  if (ev.report_count && ev.report_count > 1) {
    return 'The NOAA page lists every U.S. report that day. In the ' + table
      + ' table, look for the ' + ev.report_count + ' GA rows in ' + ev.county
      + ' County and nearby (starting around ' + t + ').';
  }
  return 'The NOAA page lists every U.S. report that day. In the ' + table
    + ' table, find the GA row' + (t ? ' at ' + t : '') + ': "' + ev.neighborhood + ', ' + ev.county + ' Co."';
}
...
function showHailPanel(ev) {
  ...
  // REP TAB \u2014 lead with exact size + category, NOAA record for the adjuster
  document.getElementById('rep-hero-size').textContent = ev.size_in.toFixed(2) + '"';
  document.getElementById('rep-hero-cat').textContent = hailCategory(ev.size_in) + ' Hail';
  setNoaaBtn('rep-noaa-btn', ev);
  setNoaaBtn('ho-noaa-btn', ev);
  document.getElementById('rep-noaa-sub').textContent =
    'Show the adjuster this official NOAA record. ' + noaaFindHint(ev, 'HAIL');
  document.getElementById('ho-noaa-sub').textContent =
    'The same federal record insurance companies check. ' + noaaFindHint(ev, 'HAIL');

  // REP TAB — storm facts only (public page: no pitch/canvassing content)
  document.getElementById('rep-storm-data').innerHTML =
    'Date: <strong style="color:#F7F2EA">' + fmtDate(ev.date) + '</strong><br>'
    + (ev.time_utc ? 'Report time: <strong style="color:#F7F2EA">' + ev.time_utc + ' UTC</strong><br>' : '')
    + 'Report location: <strong style="color:#F7F2EA">' + ev.neighborhood + ', ' + ev.county + ' County</strong><br>'
    + 'Hail size: <strong style="color:#F7F2EA">' + ev.size_in.toFixed(2) + '" (' + hailCategory(ev.size_in) + ')</strong><br>'
    + 'Coordinates: ' + ev.lat.toFixed(2) + ', ' + ev.lng.toFixed(2) + '<br>'
    + 'Estimated homes in area: ' + (ev.estimated_homes || 0).toLocaleString()
    + (ev.notes ? '<br><em style="color:rgba(247,242,234,0.4)">NOAA report: ' + ev.notes + '</em>' : '');

  document.getElementById('rep-risk').innerHTML =
    isExtreme ? 'Shingle bruising and cracks, broken vents and skylights, exposed underlayment — leaks often develop months later, and hail damage is rarely visible from the ground.'
    : isMod ? 'Cracked shingles, granule loss exposing the asphalt layer, dented gutters and soft metals — slow leaks that surface later as ceiling stains.'
    : 'Granule loss and hidden shingle bruising that shortens roof life — highest risk for roofs 10+ years old or with prior storm exposure.';

  document.getElementById('rep-insurance').innerHTML =
    'Date of loss: <strong style="color:#F7F2EA">' + fmtDate(ev.date) + '</strong><br>'
    + 'Documented hail size: <strong style="color:#F7F2EA">' + ev.size_in.toFixed(2) + '" (' + hailCategory(ev.size_in) + ')</strong><br>'
    + 'Verified NOAA SPC ground report on federal record — use the button above<br>'
    + (inWindow(ev.date) ? 'Inside the 2-year claim window' : 'Outside the typical 2-year claim window');
  ...
  if (qualifies && isExtreme) {
    ...
    ivEl.innerHTML = '⚠️ Severe — call Bailey today: (770) 381-8081';
  } else if (qualifies) {
    ivEl.style.cssText = 'background:rgba(76,175,125,0.1);color:#4CAF7D;border:1px solid rgba(76,175,125,0.25)';
    ivEl.innerHTML = '✓ Your home may qualify for an insurance inspection.';
  } else {
    ivEl.style.cssText = 'background:rgba(247,242,234,0.06);color:rgba(247,242,234,0.5);border:1px solid rgba(247,242,234,0.1)';
    ivEl.innerHTML = 'Outside the 2-year claim window — shown for history.';
  }

  // HOMEOWNER TAB — what hit, what it can cause, what Bailey handles
  document.getElementById('ho-what-hit').innerHTML =
    '<span class="ho-size">' + ev.size_in + '"</span> hail hit ' + ev.city + '<br>on ' + fmtDate(ev.date) + '.';

  document.getElementById('ho-can-cause').innerHTML =
    '<strong>What ' + hailCategory(ev.size_in).toLowerCase() + ' hail can do:</strong> '
    + (isExtreme ? 'bruised and cracked shingles, broken skylights, dented vents — damage insurers routinely approve for full roof replacement.'
      : isMod ? 'cracked shingles, granule loss, and dented gutters and vents — often enough for an approved claim.'
      : 'granule loss and hidden bruising that shortens roof life — worth a professional look, especially on roofs 10+ years old.');

  document.getElementById('ho-bailey').innerHTML =
    '<strong>What Bailey handles:</strong> free inspection, full damage documentation, and we deal with your insurance company. You pay only your deductible.';

  switchTab('rep', document.querySelector('.ptab'));
...
function showWindPanel(ev) {
  ...
  document.getElementById('wind-info').innerHTML =
    ...
    + '<strong style="color:#C8A45A">For the Insurance Claim:</strong> This wind event is documented on the NOAA federal record (button below). Homes with 3-tab shingles or roofs over 10 years old are most at risk of shingle lift and flashing damage.';

  setNoaaBtn('wind-noaa-btn', ev);
  document.getElementById('wind-noaa-sub').textContent =
    'Official federal record of this wind event. ' + noaaFindHint(ev, 'WIND');
...
async function geocode(raw) {
  ...
  const hasGa = /\b(ga|georgia)\b/i.test(raw);
  const zipMatch = raw.match(/(\d{5})(-\d{4})?\s*$/);
  const street = raw.split(',')[0]
    .replace(/(\d{5})(-\d{4})?\s*$/, '')
    .replace(/\b(ga|georgia)\b\.?\s*$/i, '')
    .replace(/[,\s]+$/, '')
    .trim();

  const attempts = [];
  // 1. What they typed (add GA only if missing)
  attempts.push('q=' + encodeURIComponent(hasGa || zipMatch ? raw : raw + ', Georgia USA'));
  // 2. Structured: street + zip + state (survives city-name mismatches in OSM)
  if (street && zipMatch) {
    attempts.push('street=' + encodeURIComponent(street) + '&postalcode=' + zipMatch[1] + '&state=Georgia');
  }
  // 3. Structured: street + state only
  if (street) {
    attempts.push('street=' + encodeURIComponent(street) + '&state=Georgia');
  }

  for (const a of attempts) {
    try {
      const res = await fetch(base + a, { headers: { 'Accept-Language': 'en' } });
...
async function searchAddress() {
  ...
  try {
    ...
    if (!hit) {
      alert('Address not found. Try just the street address (e.g. "6030 Allen Ln") or add the city (e.g. "123 Oak St, Suwanee GA").');
...
function showAddressPanel(addr, lat, lng, nearHail, nearWind) {
  ...
  if (hailInWindow.length) {
    ...
    hailInWindow.slice(0, 6).forEach(ev => {
      html += '<div class="feed-row">'
        ...
        + (ev.report_url ? '<a class="feed-noaa" href="' + ev.report_url + '" target="_blank" rel="noopener" onclick="event.stopPropagation()">NOAA</a>' : '')
        + '</div>';
    });
  ...
  // Prominent NOAA report button — anchored to the biggest storm at this address
  const topHail = hailInWindow.length
    ? hailInWindow.reduce((a, b) => (b.size_in > a.size_in ? b : a))
    : null;
  const topWind = windInWindow.length
    ? windInWindow.reduce((a, b) => (b.mph > a.mph ? b : a))
    : null;
  const topEv = topHail || topWind || nearHail[0] || nearWind[0] || null;
  setNoaaBtn('addr-noaa-btn', topEv);
  const noaaSub = document.getElementById('addr-noaa-sub');
  if (topEv && topEv.report_url) {
    noaaSub.classList.remove('hidden');
    noaaSub.textContent = (topEv.size_in !== undefined
      ? 'Official NOAA record: ' + topEv.size_in + '" hail on ' + fmtDate(topEv.date) + '. '
      : 'Official NOAA record: ' + topEv.mph + ' mph wind on ' + fmtDate(topEv.date) + '. ')
      + noaaFindHint(topEv, topEv.size_in !== undefined ? 'HAIL' : 'WIND');
  } else {
    noaaSub.classList.add('hidden');
  }
...
document.querySelectorAll('.filter-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    const group = btn.dataset.type !== undefined ? '[data-type]' : '[data-year]';
    document.querySelectorAll('.filter-btn' + group).forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    if (btn.dataset.type !== undefined) typeFilter = btn.dataset.type;
    else yearFilter = btn.dataset.year;
    renderMap();
  });
});
...
// ═══════════════════════════════════════════════════════
// INSPECTION FORM — modal on GitHub, baileyPopup on Webflow
// ═══════════════════════════════════════════════════════
function openInspectionForm(context) {
  ...
  if (modal) {
    ...
    if (context) {
      const ctx = document.getElementById('form-storm-context');
...
function closeFormIfOutside(e) {
  if (e.target === document.getElementById('inspection-modal')) {
    closeInspectionForm();
  }
}
